...
class BaseGraphProcessor:
    ...
    def build_vulnerability_info(
        ...
        for vuln in all_vulns:
            vuln_info = {
                "id": vuln.get("id", vuln.get("cve_id", vuln.get("source_id", "Unknown"))),
                "cve_id": vuln.get("cve_id", "Unknown"),
                "source_id": vuln.get("source_id", vuln.get("id", "Unknown")),
...
class NetworkGraphProcessor(BaseGraphProcessor):
    ...
    def _add_root_node(self, root_ref: str, sbom_data: dict[str, Any]):
        ...
        # If we have repository information, prefer that for naming
        repo_info = metadata.get("repository", {})
        if isinstance(repo_info, dict) and repo_info.get("name"):
...
class HierarchicalGraphProcessor(BaseGraphProcessor):
    ...
    def _process_hierarchical_dependencies(self, sbom_data: dict[str, Any], root_ref: str):
        ...
        for dep in dependencies:
            ...
            if source_ref and self.graph.has_node(source_ref):
                # Add edge from root to direct dependencies
                if self.graph.nodes[source_ref].get("type") != "LICENSE" and source_ref != root_ref:
                    self.graph.add_edge(root_ref, source_ref, relationship="direct")

                # Add hierarchical dependency edges
                for target_ref in depends_on_list:
                    if (
                        target_ref
                        and self.graph.has_node(target_ref)
                        and self.graph.nodes[target_ref].get("type") != "LICENSE"
                    ):
